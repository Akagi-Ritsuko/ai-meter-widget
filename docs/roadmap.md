...
| 2 | 通用适配器 + P0 平台 | FR-1.1, FR-1.3, FR-3, FR-4 | 🔄 代码全部完成（P2-11 智谱已全部验证通过；P2-10/12 真实验证待用户凭证） |
...
- [x] FR-1.1：智谱 GLM 配额显示正常（ZAI_REGION=cn 验证）→ **接口真实验证 + CREDIT_LIMIT 解析修复（ADR-015）+ 面板显示全部验证通过（2026-09-01，Lite 套餐数据正常）**
...
> 详细需求分解与任务计划见 [phase3-plan.md](phase3-plan.md)；统一模型契约见 [architecture.md](architecture.md) §2
...
| 2 通用适配器 + P0 | 代码完成 | 2026-08-28 | - | P2-11 智谱已全部验证通过；P2-10/12 真实验证待用户凭证 |
| 3 统一指标 API | 规划完成 | - | - | phase3-plan.md 已产出（P3-01~12），未开工 |
| 4 桌面浮窗 + 告警 | 未开始 | - | - | |
| 5 ESP32 墨水屏 | 未开始 | - | - | |
