...
> 状态：mock 验证已完成；智谱真实接口已验证（2026-09-01）；DeepSeek/OpenCode 真实验证待用户提供凭证
...
| TC-02 智谱 GLM 有效凭证 | 配额显示 | ✅ 真实接口验证（2026-09-01，`ZAI_REGION=cn`） | 接口返回 HTTP 200：Lite 套餐，5 小时窗口 0/2000（0%）、周窗口 2005/2000（100%）；发现并修复 `CREDIT_LIMIT` 解析问题（ADR-015），面板显示待重启确认 |
